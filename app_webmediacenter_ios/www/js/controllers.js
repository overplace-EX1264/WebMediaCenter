	...
	switch (id_tipologia_versione_app){
		case '2':
			...
			$scope.menu_professional = [
				...
				{ id:'youtube', name:'Youtube', link:'#/app/youtube', icon:'fa-quote-left', type:'simple', section:1, order:5, active:true, visible:true },
                    ...
                    if (!window.database.isReady){
                    	window.database.addQueue(query, function (tx, res){
                    		if ($scope.credentials.remember){
                    			...
                    			tx.executeSql(query, [response.data.id_scheda, "WebMediaCenter", "2", $scope.credentials.username, timestamp], function (){
                        			$ionicLoading.hide();
